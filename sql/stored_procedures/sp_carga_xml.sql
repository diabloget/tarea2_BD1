...
BEGIN
  ...
  BEGIN TRY
    ...
    FROM @xml.nodes('/Datos/e/error') AS T(x);
    ...
    -- Movimientos con NuevoSaldo acumulado por empleado en orden cronológico
    ;WITH Parseados AS (
      SELECT
        (SELECT Id FROM dbo.Empleado
          WHERE ValorDocumentoIdentidad = x.value('@ValorDocId', 'VARCHAR(20)'))        AS IdEmpleado,
        (SELECT Id FROM dbo.TipoMovimiento
          WHERE Nombre = x.value('@IdTipoMovimiento', 'VARCHAR(128)'))                   AS IdTipoMovimiento,
        (SELECT TipoAccion FROM dbo.TipoMovimiento
          WHERE Nombre = x.value('@IdTipoMovimiento', 'VARCHAR(128)'))                   AS TipoAccion,
        x.value('@Fecha',    'DATE')          AS Fecha,
        x.value('@Monto',    'DECIMAL(10,2)') AS Monto,
        (SELECT Id FROM dbo.Usuario
          WHERE Username = x.value('@PostByUser', 'VARCHAR(64)'))                        AS IdPostByUser,
    ...
    -- Actualizar SaldoVacaciones con el último movimiento
    UPDATE e
